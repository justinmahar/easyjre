...
export default function CopyPasteModulesContainer(props: ICopyPasteModulesContainerProps) {
  ...
  const parsePastedList = (text: string) => {
    const [jdkVersion, includedModules, excludedModules] = parseJDKModules(text);
    if (!!jdkVersion) {
  ...
  const pasteKeystroke: string = props.platform.name === MAC_OS.name ? '⌘+V' : 'Ctrl+V';
  ...
  return (
    <Container maxWidth={props.maxWidth} style={{ marginTop: '1rem', marginBottom: '1rem' }}>
      <Paper style={{ padding: '2rem' }}>
        ...
        <p>Copy a list of the available Java modules by running the following command in the JDK bin folder:</p>
        <div>
          <TextField
            variant="filled"
          ...
          {!props.jdkVersion && (
            <>
              {navigatorClipboardSupported && (
                <Button variant="contained" color="primary" onClick={() => handleClickPaste()}>
                  Paste
                </Button>
              )}
              {!navigatorClipboardSupported && (
              ...
              <Chip icon={<Icon>not_interested</Icon>} label="No JDK Modules" />
            </>
          )}
          {props.jdkVersion && (
            <>
              ...
              <Chip
                ...
                label={props.includedJDKModules.length + props.excludedJDKModules.length + ' JDK Modules'}
                color="secondary"
