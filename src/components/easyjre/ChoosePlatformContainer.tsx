...
export default function ChoosePlatformContainer(props: IChoosePlatformContainerProps) {
  ...
  const listItems: JSX.Element[] = PLATFORMS.map((platform: IPlatform, index: number) => {
    return (
      <ListItem
        key={index}
        role={undefined}
      ...
      >
        <ListItemIcon>
          <Radio edge="start" checked={props.platform.name === platform.name} tabIndex={-1} disableRipple />
        </ListItemIcon>
